...
const receiveCommand = (data, code) => {
  ...
  if(message.command === 'add') {
    ...
    return wireguard.addAccount(port, password);
  } else if (message.command === 'del') {
    const port = +message.port;
    return wireguard.removeAccount(port);
  } else if (message.command === 'list') {
    return wireguard.listAccount();
  } else if (message.command === 'pwd') {
    const port = +message.port;
    const password = message.password;
    return wireguard.changePassword(port, password);
  } else if (message.command === 'flow') {
    const options = message.options;
    return wireguard.getFlow(options);
  } else if (message.command === 'version') {
    return wireguard.getVersion();
  } else if (message.command === 'ip') {
    return wireguard.getClientIp(message.port);
  } else {
    return Promise.reject();
  }
};
